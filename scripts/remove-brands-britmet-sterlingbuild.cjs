/**
 * Remove the Britmet and Sterlingbuild brands from the primary cluster.
 *
 * Deletes, for these two brands only: their products, their menu rows
 * (categories + subcategories) and the brand records themselves. On Shopify the
 * products are ARCHIVED rather than deleted, so the store side stays
 * recoverable. Shopify collections are left alone — a collection cannot be
 * archived, only deleted, and that was not asked for.
 *
 * Every document is written to backups/ before anything is removed.
 *
 * Usage: node scripts/remove-brands-britmet-sterlingbuild.cjs [--apply]
 */
...
const NAMES = [/^britmet$/i, /^sterlingbuild$/i];
const CHUNK = 25;
...
(async () => {
  ...
  const brands = await db.collection("brands").find({ name: { $in: NAMES } }).toArray();
  if (brands.length !== 2) throw new Error(`expected 2 brands, found ${brands.length} — aborting`);
  ...
  const file = path.join(dir, `britmet-sterlingbuild-${stamp}.json`);
