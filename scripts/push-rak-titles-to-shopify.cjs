/**
 * Push the cleaned RAK CERAMICS names to Shopify as product titles.
 *
 ...
 * which strip-rak-name-prefix.cjs sets on every name it changed.
 *
 * Before writing, the current Shopify titles are read back and journalled to
 * rollback-rak-shopify-titles-<stamp>.json so the store side is reversible.
 *
 * Dry run by default.
 *   node --require ./scripts/mongo-dns.cjs scripts/push-rak-titles-to-shopify.cjs
 *   ... --apply            commit
...
(async () => {
  ...
  const rollbackPath = path.join(__dirname, "..", `rollback-rak-shopify-titles-${stamp}.json`);
  ...
  if (failures.length) {
    const failPath = path.join(__dirname, "..", `rak-shopify-title-failures-${stamp}.json`);
